...
from transformers import GPT2Tokenizer, GPT2Model, CLIPProcessor, CLIPModel, SwinForImageClassification
...
def collate_fn(batch):
    ...
    if isinstance(labels_or_keys[0], int):
        ...
        return images, gpt2_inputs, clip_text_inputs.input_ids, clip_image_inputs, labels
    else:
        keys = labels_or_keys
        return images, gpt2_inputs, clip_text_inputs.input_ids, clip_image_inputs, keys
...
class MultimodalModel(nn.Module):
    def __init__(self, gpt2_model, clip_model, swin_model, hidden_dim=512, num_classes=2):
    ...
    def forward(self, images, gpt2_input, clip_text_input, clip_image_input):
        ...
        text_features = torch.cat((gpt2_features, clip_text_features), dim=1)
...
model = nn.DataParallel(MultimodalModel(gpt2_model, clip_model, swin_model), device_ids=[1]).to(device)
...
def validate_model(model, val_loader, criterion):
    ...
    with torch.no_grad():
        for images, gpt2_input, clip_text_input, clip_image_input, labels in val_loader:
            ...
            outputs = model(images, gpt2_input, clip_text_input, clip_image_input)
...
def train_model(model, train_loader, val_loader, known_test_loader, num_epochs=20):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for images, gpt2_input, clip_text_input, clip_image_input, labels in train_loader:
            images = images.to(device)
            gpt2_input = {key: val.to(device) for key, val in gpt2_input.items()}
            clip_text_input = clip_text_input.to(device)
            clip_image_input = clip_image_input.to(device)
...
def predict_model(model, test_loader):
    ...
    with torch.no_grad():
        for images, gpt2_input, clip_text_input, clip_image_input, keys in tqdm(test_loader):
            images = images.to(device)
            gpt2_input = {key: val.to(device) for key, val in gpt2_input.items()}
            clip_text_input = clip_text_input.to(device)
            clip_image_input = clip_image_input.to(device)
