...
def collate_fn(batch):
    ...
    if isinstance(labels_or_keys[0], int):
        ...
        return images, bert_inputs, gpt2_inputs, labels
    else:
        keys = labels_or_keys
        return images, bert_inputs, gpt2_inputs, keys
...
class MultimodalModel(nn.Module):
    def __init__(self, bert_model, gpt2_model, swin_model, hidden_dim=512, num_classes=2):
    ...
    def forward(self, images, bert_input, gpt2_input):
        ...
        swin_features = self.swin_fc(self.swin_model(images).logits)

        text_features = torch.cat((bert_features, gpt2_features), dim=1)
        text_fusion = self.text_fusion_fc(text_features)
        visual_fusion = self.visual_fusion_fc(swin_features)
...
model = nn.DataParallel(MultimodalModel(bert_model, gpt2_model, swin_model), device_ids=[1]).to(device)
    ...
    def validate_model(model, val_loader, criterion):
        ...
        with torch.no_grad():
            for images, bert_input, gpt2_input, labels in val_loader:
                ...
                outputs = model(images, bert_input, gpt2_input)
...
def train_model(model, train_loader, val_loader, known_test_loader, num_epochs=20):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for images, bert_input, gpt2_input, labels in train_loader:
            images = images.to(device)
            bert_input = {key: val.to(device) for key, val in bert_input.items()}
            gpt2_input = {key: val.to(device) for key, val in gpt2_input.items()}
...
def predict_model(model, test_loader):
    ...
    with torch.no_grad():
        for images, bert_input, gpt2_input, keys in tqdm(test_loader):
            images = images.to(device)
            bert_input = {key: val.to(device) for key, val in bert_input.items()}
            gpt2_input = {key: val.to(device) for key, val in gpt2_input.items()}
